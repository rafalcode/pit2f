...
CFLAGS=-O2
EXES=lnarr lnarr_scfl strarr_scfl str2arr_scfl rdfasta pit2f pit2f_d hamin2 pit2fyu pit2fwh pit2fwh_d pit2d rd1ctf rd1ctfh
...
# After some time, come back to this looking for code to set operations on SNP names lists
# let's start with read 1-column text file
rd1ctf: rd1ctf.c
	${CC} ${DBGCFLAGS} -DDEBUG -o $@ $^
# this version hashes each line which is not zero length.
rd1ctfh: rd1ctfh.c
	${CC} ${DBGCFLAGS} -DDEBUG -o $@ $^
